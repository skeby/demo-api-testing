  ...
  if (qr) {
    if ("gender" in qr) {
      ...
      const found = authors.some(
        (author) => author.gender.toLowerCase() === gn.toLowerCase()
      );
      if (found) {
...
router.post("/", (req, res) => {
  ...
  if (
    ...
    newAuthor.name.trim().length < 4 ||
    newAuthor.name.trim().length > 25
  ) {
    return res.status(400).sendData({
  ...
  fs.writeFile(
    filePath,
    JSON.stringify(
      authors.sort((m1, m2) => m1.id - m2.id),
      null,
      2
    ),
    (err) => {
...
router.put("/:id", (req, res) => {
  ...
  if (found) {
    ...
    if (
      ...
      updAuthor.hasOwnProperty("name") &&
      (updAuthor.name.trim().length < 4 || updAuthor.name.trim().length > 25)
    ) {
      return res.status(400).sendData({
        msg: "Name should be 4 to 25 characters long",
      });
    } else if (
      updAuthor.hasOwnProperty("gender") &&
      !genders.includes(updAuthor.gender.toLowerCase())
    ) {
      return res.status(400).sendData({
        msg: "Gender should be either male or female only",
      });
